...
function createStressClient(docId: string, userId: string): StressClient {
  ...
  const ws = new WebSocket(`${wsUrl}?docId=${docId}&userId=${userId}`);
  let crdt: any = null;
  ...
  return {
    ...
    delete(index: number) {
      ...
      if (result) {
        ws.send(
          JSON.stringify({ type: "crdt-delete", payload: result.operation }),
        );
      }
    },
    getText() {
...
const engines: ("yjs" | "custom")[] = ["yjs", "custom"];
let currentPort = 4003;

engines.forEach((engine) => {
  describe(`Stress Tests [Engine: ${engine}]`, () => {
    beforeAll(() => {
      config.crdtEngine = engine;
      const port = currentPort++;
      const testServer = createTestServer(port);
    ...
    afterEach(() => {
      if (clearDocuments) clearDocuments();
    });
      ...
      test("convergence with random inserts and deletes", async () => {
        ...
        for (let i = 0; i < 15; i++) {
          if (client1.getText().length > 0) {
            client1.delete(
              Math.floor(Math.random() * client1.getText().length),
            );
          }
          if (client2.getText().length > 0) {
            client2.delete(
              Math.floor(Math.random() * client2.getText().length),
            );
          }
        }
    ...
    describe("performance benchmarks", () => {
      test("measures time for 500 sequential inserts", async () => {
        ...
        console.log(
          `[${engine}] 500 sequential inserts: ${duration.toFixed(2)}ms`,
        );
      ...
      test("measures broadcast latency with 5 clients", async () => {
        ...
        console.log(
          `[${engine}] Broadcast latency (5 clients, 50 ops): ${duration.toFixed(2)}ms`,
        );
